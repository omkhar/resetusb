...
static ssize_t fake_libusb_get_device_list(libusb_context *ctx, libusb_device ***list) {
    (void)ctx;
...
static void fake_libusb_free_device_list(libusb_device **list, int unref_devices) {
    (void)list;
    (void)unref_devices;
...
static int fake_libusb_get_device_descriptor(libusb_device *device, struct libusb_device_descriptor *desc) {
    int idx = index_for_device(device);
    if (idx < 0) {
...
static int fake_libusb_open(libusb_device *device, libusb_device_handle **handle) {
    int idx = index_for_device(device);
    if (idx < 0) {
        return LIBUSB_ERROR_OTHER;
    }
...
static int fake_libusb_get_string_descriptor_ascii(libusb_device_handle *handle,
    ...
    if (g_fake.string_rc[idx] > 0 && data != NULL && length > 0) {
        const char *value = g_fake.string_value[idx] == NULL ? "" : g_fake.string_value[idx];
...
static const resetusb_ops fake_ops = {
    ...
    .libusb_get_string_descriptor_ascii = fake_libusb_get_string_descriptor_ascii,
...
static void test_init_failure(void) {
    ...
    assert(out != NULL && strcmp(out, "") == 0);
    assert(err != NULL && strstr(err, "Failed to initialize libusb: LIBUSB_ERROR_ACCESS") != NULL);
...
static void test_successful_reset(void) {
    ...
    assert(out != NULL && strstr(out, "reset bus 1 device 2 (1234:5678) Keyboard") != NULL);
...
static void test_mixed_failures_counted(void) {
    ...
    assert(out != NULL && strstr(out, "Summary: reset 0 device(s), 2 failure(s)") != NULL);
    assert(err != NULL &&
           strstr(err, "Failed to read descriptor for bus 1 device 2: LIBUSB_ERROR_IO") != NULL);
    assert(err != NULL && strstr(err, "Can't open bus 3 device 4 (abcd:ef00): LIBUSB_ERROR_NOT_FOUND") != NULL);
...
static void test_reset_failure_closes_handle(void) {
    ...
    assert(rc == 1);
    assert(g_fake.close_calls[0] == 1);
    assert(out != NULL && strstr(out, "Summary: reset 0 device(s), 1 failure(s)") != NULL);
    assert(err != NULL &&
           strstr(err, "Failed reset bus 1 device 2 (1234:5678) <string unavailable>: LIBUSB_ERROR_IO") != NULL);
