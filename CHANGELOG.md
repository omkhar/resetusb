...
- Enforce public-repo hygiene and control-plane drift in `make lint`, including checks for leaked local paths, internal-only references, scratch detritus, and unsynchronized generated agent surfaces.
...
- Enforce reviewable pull requests in CI by rejecting PRs over 20 changed files or 700 total changed lines, matching the repo policy for small, human-reviewable changes.
